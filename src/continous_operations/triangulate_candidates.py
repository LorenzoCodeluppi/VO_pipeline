...
from utils.utility_tools import calculate_avarage_depth, get_landmark_treshold, get_angle_bearing
...
def filter_triangulated_points(points_3d, M1, M2, K, candidates, first_obs_candidates, current_R, current_t):
  ...
  reprojection_errors = np.linalg.norm(first_obs_candidates.T - projected_points_1, axis=1) + np.linalg.norm(candidates.T - projected_points_2, axis=1)
...
def triangulate_points(state: State, current_R, current_t, K, triangulate_signal):
  ...
  print("Average depth of landmarks:", avg_depth)
  ...
  print(np.round(np.sort(angles)[::-1], 1)[:10])

  mask = np.logical_or(distances > distance_threshold, np.abs(angles) > angle_treshold)
  print("Triangulations from angles:", np.sum(angles > angle_treshold))
  possible_new_landmarks = np.sum(mask)
  print("Possible new Landmarks:", possible_new_landmarks)
